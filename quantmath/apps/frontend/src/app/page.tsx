...
export default function Home(): React.JSX.Element {
  ...
  const handleCompute = (e: React.FormEvent) => {
    ...
    const ohlc = rawHistory.map((h) => ({
      x: h.time,
      y: [h.open * qty, h.high * qty, h.low * qty, h.close * qty] as [number, number, number, number],
    }));
      ...
      const safePred: StockPredictionAug = {
        ...
        confidence:
          typeof pred?.confidence === "number"
            ? Math.min(Math.max(pred.confidence, 0), 1) * 100
            : 50,
      };
            ...
            <div className="mt-4 flex gap-4">
              {["BUY", "SELL", "HOLD"].map((action) => (
                <button
                  key={action}
                  onClick={() => {
                    console.log(`${action} clicked for ${aiPrediction.symbol} with confidence ${aiPrediction.confidence}%`);
                    alert(`Action: ${action} for ${aiPrediction.symbol} executed!`);
                  }}
                  className={`flex-1 py-2 font-bold rounded transition transform hover:scale-105 focus:outline-none ${
                    action === "BUY"
                ...
                >
                  {action} ({Math.round(aiPrediction.confidence ?? 50)}%)
                </button>
              ))}
            </div>
